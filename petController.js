...
exports.createVaccineRecord = (req, res) => {
  const { id: userId } = req.user;
  const { petId } = req.params;
  const { vaccine_name, vaccine_date, next_date, hospital, completed, on_time, notes } = req.body;
  ...
  db.get('SELECT * FROM pets WHERE id = ? AND user_id = ?', [petId, userId], (err, pet) => {
    ...
    const id = uuidv4();
    const completedInt = (completed === true || completed === 'true' || completed === 1 || completed === '1') ? 1 : 0;
    const onTimeInt = (on_time === false || on_time === 'false' || on_time === 0 || on_time === '0') ? 0 : 1;
    
    db.run(
      'INSERT INTO vaccine_records (id, pet_id, vaccine_name, vaccine_date, next_date, hospital, completed, on_time, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [id, petId, vaccine_name, vaccine_date, next_date, hospital, completedInt, onTimeInt, notes],
      function(err) {
        if (err) {
          return res.status(500).json({ message: '服务器内部错误' });
        }
...
exports.updateVaccineRecord = (req, res) => {
  ...
  db.get('SELECT * FROM pets WHERE id = ? AND user_id = ?', [petId, userId], (err, pet) => {
    ...
    if (completed !== undefined) {
      ...
      updateValues.push((completed === true || completed === 'true' || completed === 1 || completed === '1') ? 1 : 0);
    }
    if (on_time !== undefined) {
      updateFields.push('on_time = ?');
      updateValues.push((on_time === false || on_time === 'false' || on_time === 0 || on_time === '0') ? 0 : 1);
    }
    if (notes !== undefined) {
      updateFields.push('notes = ?');
      updateValues.push(notes);
    ...
    db.run(
      `UPDATE vaccine_records SET ${updateFields.join(', ')}, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND pet_id = ?`,
      updateValues,
      function(err) {
        if (err) {
          return res.status(500).json({ message: '服务器内部错误' });
